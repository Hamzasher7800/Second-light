...
function validateRequest(data: ProcessDocumentRequest): void {
  if (!data.documentId) {
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a medical document analyzer. Extract key information from the document and provide a response in JSON format. Do not include any markdown formatting or backticks. The response should be a valid JSON object with this structure:\n" +
            "{\n" +
    ...
    if (analysis.key_findings && analysis.key_findings.length > 0) {
      const { error: findingsError } = await supabaseClient
        .from("key_findings")
        .upsert(
          analysis.key_findings.map((finding: any) => ({
            document_id: documentId,
